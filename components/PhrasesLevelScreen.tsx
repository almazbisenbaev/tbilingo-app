...
interface PhrasesLevelScreenProps {
  courseId: string;
  levelIcon?: string;
}

export default function PhrasesLevelScreen({
  courseId,
  levelIcon = '/images/icon-phrases.svg',
}: PhrasesLevelScreenProps) {
  const router = useRouter();
  const { currentUser } = useAuth();
  const { title: LEVEL_TITLE, description: LEVEL_DESCRIPTION } = useLevelMeta(courseId);
    ...
    const fetchItems = async () => {
      try {
        ...
        const fetchedItems = snapshot.docs.map(docSnap => {
          ...
          const id =
            typeof docSnap.id === 'string' && !isNaN(Number(docSnap.id))
              ? parseInt(docSnap.id)
  ...
  // Initialize phrases memory for items not yet in progress
  useEffect(() => {
  ...
  const startGameplay = () => {
    const unlearnedItems = items.filter(
      item => !learnedIds.some(id => String(id) === String(item.id)),
    );
    ...
    const selected = shuffleArray(unlearnedItems).slice(0, 10);
  ...
  const startReviewSession = () => {
    const selected = shuffleArray([...items]).slice(0, 10);
    setSessionItems(selected);
    setCurrentIndex(0);
    setSessionLearnedIds([]);
    setSessionReviewedIds([]);
  ...
  const handlePhraseCorrect = async () => {
    ...
    if (currentUser) {
      try {
        ...
        if (nextLearned && !updatedIds.includes(String(currentItem.id))) {
          updatedIds.push(String(currentItem.id));
        }

        const updatedItemProgress = {
          ...currentItemProgress,
          [String(currentItem.id)]: nextCorrect,
        };

        const isFinished =
          currentDoc?.isFinished === true ||
          (items.length > 0 && updatedIds.length >= items.length);
  ...
  const progressPercent =
    items.length > 0 ? Math.round((learnedIds.length / items.length) * 100) : 0;
        ...
        <View className="w-full max-w-[640px] self-center p-6">
          <View className="items-center mb-8 mt-4">
            <View className="w-24 h-24 mb-4">
              <Image
                source={imageMap[levelIcon] || require('../assets/images/icon.png')}
