...
export default function LevelNumbersScreen() {
  ...
  const [sessionItems, setSessionItems] = useState<NumberItem[]>([]);
      ...
      try {
        ...
        const fetchedItems = snapshot.docs.map(docSnap => {
          ...
          return {
            ...
            translationLatin: data.translationLatin,
            ...data
          } as NumberItem;
        });
  ...
  const startGameplay = () => {
    ...
    if (unlearnedItems.length === 0) {
      Alert.alert(
        "Course Completed", 
        "You have learned all numbers! Starting a review session.",
        [
    ...
    // Numbers: Learn in order, no shuffle
    // Select up to 10 items, or fewer if not enough available
    const selected = unlearnedItems.slice(0, 10);
      ...
      <ScrollView className="flex-1">
        ...
        <View className="bg-card p-6 rounded-2xl border-2 border-border mb-8">
          ...
          <Text className="mt-3 text-center text-sm text-muted-foreground">
            {learnedIds.length} / {items.length} numbers learned
          </Text>
        </View>
