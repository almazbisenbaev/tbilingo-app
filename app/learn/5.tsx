...
const LEVEL_DESCRIPTION = 'Common greetings and farewells';
const LEVEL_ICON = '/images/icon-phrases.svg';

export default function LevelGreetingsScreen() {
  ...
  // Sync phrases memory with items once both loaded
  useEffect(() => {
  ...
  const handlePhraseWrong = async () => {
      ...
      if (auth.currentUser) {
          try {
              ...
              // Sticky isFinished: if it was true, keep it true. Otherwise check count.
              const isFinished = currentDoc?.isFinished === true || (items.length > 0 && updatedIds.length >= items.length);
